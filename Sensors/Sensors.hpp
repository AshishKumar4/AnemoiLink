...
public:
	RPCSensorBackend(std::string ip = "0.0.0.0", int portBase = 8400) : SensorBackend<dataType>()
	{
		this->rpcStub = new rpc::client(ip, portBase - 1);
	}
	RPCSensorBackend(rpc::client *rpcStub) : SensorBackend<dataType>()
	{
...
public:
	DroneIMU_t();
	DroneIMU_t(SensorBackend<data_imu_t> *backDesc);
...
public:
	DroneGPS_t();
	DroneGPS_t(SensorBackend<GeoPoint_t> *backDesc);
...
public:
	DroneAltimeter_t();
	DroneAltimeter_t(SensorBackend<float> *backDesc);
	float fetchData();
};
...
public:
	DroneVelometer_t();
	DroneVelometer_t(SensorBackend<vector3D_t> *backDesc);
...
public:
	DroneStateSensor_t();
	DroneStateSensor_t(SensorBackend<DroneState_t> *backDesc);
